...
import { ChevronLeft, ChevronRight, Loader2, X, Sparkles, Plus, Wand2 } from "lucide-react";
                  ...
                  {dayWorkouts.slice(0, 3).map((w) => {
                    ...
                    return (
                      <div key={w.id} className="flex items-center gap-1 text-[10px] truncate">
                        ...
                        <span className={cn("truncate text-foreground/80", w.status === "completed" && "line-through text-muted-foreground")}>
                          {w.title}
                        </span>
                      </div>
                    );
              ...
              {selectedWorkouts.map((w) => {
                ...
                return (
                  <div key={w.id} className="rounded-lg border bg-secondary/20 p-4" style={{ borderColor: c.hex + "40" }}>
                    ...
                    {w.structure && <pre className="mt-2 p-3 rounded-md bg-background text-xs font-mono whitespace-pre-wrap text-secondary-foreground">{w.structure}</pre>}
                    {w.ai_adjustment_reason && (
                      <div className="mt-2 flex items-start gap-2 p-2.5 rounded-md bg-primary/5 text-xs text-primary">
                        <Sparkles className="h-3.5 w-3.5 shrink-0 mt-0.5" /> {w.ai_adjustment_reason}
                      </div>
                    )}
                  </div>
                );
